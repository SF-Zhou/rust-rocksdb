...
use sfzhou_rocksdb::{Direction, IteratorMode, MemtableFactory, Options, DB};
...
fn test_prefix_iterator() {
    let n = DBPath::new("_rust_rocksdb_prefix_iterator_test");
    {
        ...
        let prefix_extractor = sfzhou_rocksdb::SliceTransform::create_fixed_prefix(3);
...
fn test_prefix_iterator_uses_full_prefix() {
    ...
    let path = DBPath::new("_rust_rocksdb_prefix_iterator_uses_full_prefix_test");
    {
        ...
        let prefix_extractor = sfzhou_rocksdb::SliceTransform::create_fixed_prefix(1);
...
fn test_iter_range() {
    ...
    fn test(
        db: &DB,
        mode: IteratorMode,
        range: impl sfzhou_rocksdb::IterateBounds,
        want: std::ops::Range<usize>,
        reverse: bool,
    ) {
        let mut ro = sfzhou_rocksdb::ReadOptions::default();
    ...
    fn prefix(key: &[u8]) -> sfzhou_rocksdb::PrefixRange<&[u8]> {
        sfzhou_rocksdb::PrefixRange(key)
    }
    ...
    // Test Start and End modes
    {
        ...
        where
            R: sfzhou_rocksdb::IterateBounds + Clone,
        {
